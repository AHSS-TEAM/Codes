...
#define DIST_TOLERANCE 1.0

const int GPS_BAUD = 9600;
//float Source_LAT = 12.917147, Source_LON = 77.575347;
float lat_point[] = {12.917069};
float lng_point[] = {77.525309};
byte index = 0;  // Index to the current waypoint in array
int num_waypoints = sizeof(lat_point) / sizeof(lat_point[0]); 
float DEST_LAT = 12.917069, DEST_LON = 77.525309;
...
{
  gps_data();
  path_distance = TinyGPSPlus::distanceBetween(gps.location.lat(), gps.location.lng(),lat_point[index], lng_point[index]);
...
//  gps_data();
  path_heading = TinyGPSPlus::courseTo(gps.location.lat(), gps.location.lng(), lat_point[index], lng_point[index]);
